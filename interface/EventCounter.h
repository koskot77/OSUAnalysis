...
public:
    ...
    unsigned long sumThirdDimension(unsigned int dimension1, unsigned int dimension2) const{
        double sum = 0.0;
        for(unsigned int third = 0; third < dim3; ++third){
            sum += weightedEntries[dimension1][dimension2][third];
        }
        return ((unsigned long) (sum + 0.5));
    }

    unsigned long sumThirdDimensionUnweighted(unsigned int dimension1, unsigned int dimension2) const{
            unsigned long sum = 0;
            for(unsigned int third = 0; third < dim3; ++third){
